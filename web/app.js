...
async function cargarAjustes() {
  ...
  $('consejos').innerHTML = '<ul>' + [
    ...
    '<b>Los espacios cuentan.</b> «ens » con espacio final busca la sigla suelta; sin él casaría dentro de «defensa» o «bienes».',
...
function leerAjustes() {
  ...
  // recortado a «ens» casaba dentro de "defensa", "bienes" o "ensayo" y metía mil
  // licitaciones de más. Solo se descartan las líneas vacías y el retorno de carro.
  const lineas = (t) => t
    .split('\n')
    .map((s) => s.replace(/\r/g, ''))
    .filter((s) => s.trim() !== '');
...
function mostrarVista(vista) {
  for (const v of ['bandeja', 'vencimientos', 'adjudicatarios', 'ajustes']) {
